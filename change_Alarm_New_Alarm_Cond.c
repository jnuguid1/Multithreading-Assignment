...
int main(int argc, char *argv[])
{
	...
	status = pthread_create(&monitor_thread, NULL, alarm_monitor_thread, NULL);
	if (status != 0)
	...
	while (1)
	{
		...
		if (sscanf(line, "%s (%d): %s (%d) %d %64[^\n]",
				request, &alarmid, group_keyword, &groupid, &seconds, message) < 6)
		{
		...
		else
		{
			...
			else
			{
				if (strcmp(request, "Start_Alarm") == 0)
				{
					...
					if (avail_displays == 0)
					{
						status = pthread_create(&display_alarm_t, NULL, display_thread, NULL);
						if (status != 0)
							err_abort(status, "Create alarm thread");
				...
				else if (strcmp(request, "Change_Alarm") == 0)
				{
					...
					alarm->groupid = groupid;
					alarm->time = time(NULL) + alarm->seconds;
		       //inserting changed alarm back into list
					alarm_insert(alarm);
					printf("Alarm(%d) has successfully been changed.\n",
						alarmid, main, time(NULL), groupid, alarm->time, message);
					new_alarm = alarm;
				}
				else
